...
class createTable(databaseBaseClass, object):
    ...
    def createLogTable(self):
        if self.checkTable(self.log):
            count = 1
            while True:
                if self.checkTable(self.log + str(count)):
                    count += 1
                else:
                    query = u"alter table {0} rename to {1}".format(self.log, self.log + str(count))
                    self.execute(query)
                    self.logger.info(u"Rename old log table {0}".format(self.log + str(count)))
                    break

        # We need a workaround to allow use of format for header names,
        # and also later for '{0}' on first line
        query = u"create table {0}" + u"(time varchar(32) not NULL, team varchar(16) not NULL, step varchar(16) not NULL, result varchar(8) not NULL, {0} integer not NULL)".format(
    ...
    def createScenario(self, scenario, firstStep):
        # We need a workaround to allow use of format for header names,
        # and also later for '{0}' on first line
        query = u"create table {0}" + u"({0} varchar(16) not NULL, {1} varchar(256) not NULL, {2} varchar(16) not NULL, {3} varchar(16) not NULL)".format(
            Storyboard.STEP_KEY, Storyboard.LABEL_KEY, Storyboard.SUCCESS_KEY, Storyboard.FAILURE_KEY)
