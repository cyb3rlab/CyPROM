...
def createReport():
    ...
    for team in teams:
        ...
        with open("{0}.csv".format(team_name), "w") as f:
            ...
            f.write("# Start time:\t{0}\n# End time:\t{1}\n# Final score:\t{2}\n".format(encode(first_rec[LOG_TIME_COL]),
                                                                                         encode(last_rec[LOG_TIME_COL]),
                                                                                         str(team_score)))
            ...
            for record in logs:
                if SKIP_UNAVAILABLE and record[LOG_OUTCOME_COL] == "Unavailable":
                    continue
                f.write("{0},{1},{2},{3}\n".format(encode(record[LOG_TIME_COL]), encode(record[LOG_STEP_COL]),
                                                   record[LOG_OUTCOME_COL], record[LOG_SCORE_COL]))
