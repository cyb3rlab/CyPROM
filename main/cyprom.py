...
from init_config import init_config, init_targets, init_cyris_targets, init_scenario, init_targets_by_team_name
...
def main():
    ...
    # args --team_name to choose a certain team
    if args.team_name:
        if args.target:
            teamConfig = init_targets_by_team_name(args.target, args.team_name)
        else:
            teamConfig = init_targets_by_team_name(config[Storyboard.TARGET_FILE_KEY], args.team_name)
    ...
    if config["LogMode"] == "low":
        ...
        logger.info(
            "\033[37;44;1m{0:^8} | {1:^7} | {2:^5} | {3:^16} | {4: <16} |\033[0m".format("TIME", "LEVEL", "PID", "TEAM",
                                                                                         ""))
    else:
        logger.info(
            "\033[37;44;1m{0:^23} | {1:^7} | {2:^5} | {3:^16} | {4: <16} |\033[0m".format("TIME", "LEVEL", "PID",
                                                                                          "TEAM", ""))
