...
from check_target_file import check_target_file, is_exist_team_name, is_exist_team_number
...
def init_targets_by_team_name(target_file, team_name):
    # Read target information file and change type from list to dictionary
    logger.info("Read target information file '{0}' --- Start".format(target_file))
    target_config = read_ini_file(target_file)
    ...
    # Check target_config
    if not check_target_file(target_config, team_name):
        print("[-] init_config: ERROR: Target information file is invalid: '{0}'.".format(target_file))
        return False

    # check if the team args is a number or a character
    if team_name.isnumeric():
        # check if team number exist
        if not is_exist_team_number(target_config, team_name):
            print("[-] init_config: ERROR: Target information file does not include team '{0}'.".format(team_name))
            return False
    else:
        # check the team name exist
        if not is_exist_team_name(target_config, "Team " + team_name):
            print("[-] init_config: ERROR: Target information file does not include team '{0}'.".format(team_name))
            return False

    logger.info("Read target information file '{0}' --- Complete".format(target_file))

    # extract teams into items
    teams = target_config.items()
    # get the chosen team by name or number
    if team_name.isnumeric():
        # searching for chosen team, by integer number
        chosen_team = next((item for item in teams if item[0] == team_name), teams)
    else:
        # searching for chosen team, both uppercase and lowercase
        chosen_team = next((item for item in teams if item[0].lower() == ("Team " + team_name).lower()), teams)
    # cast chosen team to dict
    target_config = dict({chosen_team[0]: chosen_team[1]})
...
def init_cyris_targets(cyris_file, test):
    ...
    for host in range_details["hosts"]:
        for instance in host["instances"]:
            ...
            participant_name = u"CR{0}_{1}".format(str(range_id), str(instance_index))
            if participant_name not in participant_list:
                target_config[participant_name] = {}

            for i, guest in enumerate(instance["guests"], 1):
                guest_id = u"{0}{1}".format(decode(guest["guest_id"]), str(i))
