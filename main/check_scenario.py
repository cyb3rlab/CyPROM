...
def check_module(options, module):
    ...
    if Storyboard.MODULE_KEY in options:
        ...
        if not moduleClass.check(options):
            print("[-] {0}: An error was detected in the settings of module '{1}'.".format(module, options[
                Storyboard.MODULE_KEY]))
            return False
        del moduleClass
    else:
        print(options)
...
def check_result(scenarioResult, steps, fileList):
    ...
    if isinstance(scenarioResult, dict):
        ...
        if Storyboard.NEXT_KEY in scenarioResult:
            ...
            if (scenarioResult[Storyboard.NEXT_KEY] not in steps) and (
                    scenarioResult[Storyboard.NEXT_KEY] not in fileList):
                print("[-] check_scenario: Key '{0}' value is not a known step id or scenario file: '{1}'.".format(
                    Storyboard.NEXT_KEY, scenarioResult[Storyboard.NEXT_KEY]))
                return False
        ...
        if Storyboard.NEXT_SCENARIO_KEY in scenarioResult:
            ...
            if not check_string(scenarioResult[Storyboard.NEXT_SCENARIO_KEY]):
                print(
                    "[-] scenario: Value is not a string: '{0}'.".format(scenarioResult[Storyboard.NEXT_SCENARIO_KEY]))
                return False

            # Check whether the scenario file exists
            if scenarioResult[Storyboard.NEXT_SCENARIO_KEY] not in fileList:
                print("[-] scenario: Value is not a known scenario file: '{0}'.".format(
                    scenarioResult[Storyboard.NEXT_SCENARIO_KEY]))
                return False
        ...
        if Storyboard.POINTS_KEY in scenarioResult:
            ...
            if not isinstance(scenarioResult[Storyboard.POINTS_KEY], int):
                print(
                    "[-] check_scenario: ERROR: Key '{0}' value is not an integer: '{1}'".format(Storyboard.POINTS_KEY,
                                                                                                 scenarioResult[
                                                                                                     Storyboard.POINTS_KEY]))
                return False
...
def check_scenario(scenario, targetList, fileList):
    ...
    for STEP in scenario:
        ...
        for key in required_keys:
            if key not in STEP:
                print("[-] check_scenario: ERROR: Step '{0}' doesn't contain required key: '{1}'".format(STEP["step"],
                                                                                                         key))
                return False
        ...
        for result in outcome_keys:
            ...
            if result in STEP:
                if not check_result(STEP[result], steps, fileList):
                    print("[-] check_scenario: Step '{0}' contains an incorrect value for the result key '{1}'.".format(
                        STEP["step"], result))
                    return False
